...
def main():
    ...
    #generation(canvas, x0,y0,*rotpi3(base))
    last_point = third_point(base)
    #create_line(canvas, *base)
    #create_line(canvas, x0, y0, last_point[0],last_point[1])
    #canvas.create_line(x0,y0,last_point[0],last_point[1],fill="black", width = 3)
    new_base = (x0,y0,*third_point(base))
    new_third_point = third_point(new_base)
    trisect = middle_points(base)
    '''
    
    '''
    create_line(canvas,trisect[0],trisect[1],*third_point(trisect))

    #create_line(canvas,x0,y0,*new_third_point)
    root.mainloop()
